package com.social.utils;

public class UrlsDictionary {
    public static final String VK_POST_WALL = "api.vk.com/method/wall.post";
    
    public static final String VK_REDIRECT_URL = "https://oauth.vk.com/blank.html";
    public static final String VK_OAUTH_DIALOG = "https://oauth.vk.com/authorize";

    public static final String FB_GRAPH = "graph.facebook.com/";
    public static final String FB_POST_WALL = "/feed";
    public static final String FB_OAUTH_DIALOG = "https://www.facebook.com/dialog/oauth";
    public static final String FB_REDIRECT_URL = "http://www.i.ua";
    
    public static final String OK_OAUTH_DIALOG = "https://connect.ok.ru/oauth/authorize";
    public static final String OK_REDIRECT_URL = "http://www.i.ua";
    public static final String OK_URL_REQUEST = "http://api.ok.ru/fb.do";
    public static final String OK_GET_FRIENDS = "http://api.ok.ru/api/friends/get";
    
}
